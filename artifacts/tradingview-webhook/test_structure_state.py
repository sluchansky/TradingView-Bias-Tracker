...
from structure_state import (
    STRUCTURE_CANDIDATE_POINTS,
    STRUCTURE_INITIAL_POINTS,
    STRUCTURE_POINTS,
    resolve_structure_cycle,
)
...
def test_first_choch_is_reversal_candidate_with_reduced_credit():
...
def test_neutral_bos_establishes_initial_directional_structure_only():
    state = resolve([event("BOS SUPPLY", 1)])
    assert state["state"] == "TREND_INITIAL"
    assert state["direction"] == "Short"
    assert state["confirmed"] is False
    assert state["allocation_points"] == STRUCTURE_INITIAL_POINTS
    assert state["next_event"] == "BOS SUPPLY"


def test_second_aligned_bos_confirms_initial_trend_continuation():
    state = resolve([event("BOS SUPPLY", 4), event("BOS SUPPLY", 1)])
    assert state["state"] == "TREND_CONFIRMED"
    assert state["direction"] == "Short"
    assert state["confirmed"] is True
    assert state["allocation_points"] == STRUCTURE_POINTS
    assert state["next_event"] == "CHOCH DEMAND"


def test_repeated_continuation_bos_preserves_one_confirmed_allocation():
    state = resolve([
        event("BOS DEMAND", 6),
        event("BOS DEMAND", 4),
        event("BOS DEMAND", 1),
    ...
    assert state["state"] == "TREND_CONFIRMED"
    assert state["events_in_cycle"] == 3
    assert state["allocation_points"] == STRUCTURE_POINTS
...
def test_same_direction_choch_cannot_supersede_initial_trend_structure():
    state = resolve([event("BOS DEMAND", 4), event("CHOCH DEMAND", 1)])
    assert state["state"] == "TREND_INITIAL"
    assert state["direction"] == "Long"
    assert state["last_event"] == "BOS DEMAND"
    assert state["events_in_cycle"] == 1
