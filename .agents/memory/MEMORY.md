...
- [Edge Score card block](edge-score-card-block.md) — display-only ⚡Edge Score block (replaces AI Analysis); app is alert-driven (no OHLC) so price-action signals are new alert types — Liquidity Sweep wired as {MGC,MNQ}{BULLISH,BEARISH} SWEEP (side=sweep,score 0); never fabricate a label.
